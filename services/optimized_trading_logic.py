...
class OptimizedTradingLogic:
    ...
    def __init__(self, config=None):
        ...
        # Version 2.3.0 - 4時間足最適化（機会損失削減と損失抑制のバランス）
        self.regime_params = {
            'TRENDING': {
                'rsi_oversold': 40,      # 押し目買い
                'rsi_overbought': 60,    # 戻り売り
                'signal_threshold': 1.0,  # 1.2 → 1.0（4時間足では厳しすぎる）
                'stop_loss_atr_mult': 1.5,  # 損失削減
                'take_profit_atr_mult': 3.5,  # 早めの利確
            },
            'RANGING': {
                ...
                'stop_loss_atr_mult': 1.5,  # 損失削減
                'take_profit_atr_mult': 3.0,  # 早めの利確
            },
            'VOLATILE': {
                'rsi_oversold': 35,
                'rsi_overbought': 65,
                'signal_threshold': 1.5,  # 1.7 → 1.5（機会損失削減）
                'stop_loss_atr_mult': 2.0,  # 損失削減
                'take_profit_atr_mult': 4.0,  # 早めの利確
            }
        }
    ...
    def should_trade(self, market_data, historical_df=None, skip_price_filter=False, is_tpsl_continuation=False):
        ...
        try:
            ...
            if not skip_price_filter:
                ...
                # 最小価格変動チェック（手数料負け防止 - バランス設定）
                # v2.7.1: 0.7%に設定（機会損失削減と往復ビンタ防止のバランス）
                if self.last_trade_price is not None:
                    price_change_ratio = abs(current_price - self.last_trade_price) / self.last_trade_price
                    if price_change_ratio < 0.007:  # 0.7%未満の変動では取引しない
                        logger.info(f"⏸️ Price hasn't moved enough ({price_change_ratio*100:.2f}% < 0.7%) - waiting...")
