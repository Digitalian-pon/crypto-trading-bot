#!/usr/bin/env python3
"""
Fixed standalone script to start the trading bot
This properly handles Flask app context and database sessions
"""
...
def main():
    ...
    try:
        ...
        with app.app_context():
            ...
            # Create and start trading bot with app context
            logger.info("Creating fixed trading bot instance...")
            trading_bot = FixedTradingBot(
                user=user, 
                api_key=user.api_key, 
                api_secret=user.api_secret, 
                app=app  # Pass the Flask app for context
            )
            trading_bot.set_db_session(db.session)
            
            logger.info("Starting fixed trading bot...")
            if trading_bot.start(interval=60):
                logger.info("Fixed trading bot started successfully!")
                ...
                try:
                    ...
                    trading_bot.stop()
                    logger.info("Fixed trading bot stopped.")
                    
            else:
                logger.error("Failed to start fixed trading bot!")
                return False
